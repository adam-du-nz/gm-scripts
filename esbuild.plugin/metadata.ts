...
function formatField(key: string, value: Metadata.CustomValue): FormattedProperty[] {
  if (typeof value === "string") {
    return [[key, value]];
  }

  if (Array.isArray(value)) {
    const uniqArray = unique(value);
    return uniqArray.map((item) => [key, item]);
  }
...
function normalizeResources(resources: string[] | Record<string, string>): string[] {
...
export function formatMetadata(metadata: Metadata) {
  ...
  for (const [key, value] of Object.entries(metadata)) {
    ...
    if (key === "resource") {
      formattedProperties.push(...formatField(key, normalizeResources(value as Record<string, string>)));
